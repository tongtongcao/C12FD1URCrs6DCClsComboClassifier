...
class ClusterDataset(Dataset):
    ...
    Each row of the input CSV is expected to have 14 features:
        ...
        - Next 6 columns: slope values
        - Last 2 columns: x, y for uRWell crosses (normalized by x, y ranges)
    ...
    def __init__(self, csv_files):
        ...
        # uRWell features (x, y)
        X_urwell = df.iloc[:, 12:14].values.astype(np.float32)
        # Normalize x and y
        X_urwell[:, 0] = (X_urwell[:, 0]) / 140.0  # x
        X_urwell[:, 1] = X_urwell[:, 1] / 160.0  # y

        self.X = np.concatenate([X_avg, X_slope, X_urwell], axis=1)
